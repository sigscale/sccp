...
-export([party_address/1, nai/1, tt/1, numbering_plan/1, encoding_scheme/1,
		...
		segmenting/1, return_cause/1, segmentation/1]).
...
sccp(<<?UnitData, Class, CalledPartyP, CallingPartyP, DataP, Rest/binary>>) ->
	...
	DataL = binary:at(Rest, DataP - 1),
	CalledPartyB = binary:part(Rest, CalledPartyP, CalledPartyL),
	CallingPartyB = binary:part(Rest, CallingPartyP, CallingPartyL),
	Data = binary:part(Rest, DataP, DataL),
...
refusal_cause(6) -> network_resource;
refusal_cause(7) -> network_resource;
...
refusal_cause(C) when is_integer(C), C > 20 -> reserved;
...
refusal_cause(network_resource) -> 6;
...
-spec point_code(P1) -> P2
	when
		P1 :: undefined | integer(),
		P2 :: binary().
